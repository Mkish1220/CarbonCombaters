...
const questionsFormHandler = async (event) => {
  ...
  if (gasoline || diesel || electric || miles || hours || nattie || liquid || fuel || recycle || bbq || phones ||
    ...
    const response = await fetch('/api/newquestions', {
      method: 'POST',
      body: JSON.stringify({
        gasoline, diesel, electric, miles, hours, nattie, liquid, fuel, recycle,
        bbq, phones, water, meat, average, nobeef, vegetarian, vegan, user_id
      }),
...
document.getElementById("questionForm").addEventListener("click", questionsFormHandler);